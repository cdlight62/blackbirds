...
export const updateItems = async (...updateDefinitions) => {
  for (let d of updateDefinitions) {
    const pack = game.packs.get(d.packId);
    const packItems = await pack.getDocuments({ type: d.itemType });
  ...
  const updateItem = async (item, source) => {
    for (let { packItemMap, dataPathsToUpdate } of updateDefinitions.filter((d) => d.itemType === item.type)) {
      ...
      if (!packItem) return;
      console.log(
        `Updating item "${item.name}" in ${source} (id: ${item._id}) from pack "${packItem.pack}" (Item "${packItem.name}")...`
      );
      ...
      for (let i of t?.actor?.items) {
        if (!t.isLinked) {
          const update = await updateItem(
            i?.toObject?.() ?? i,
            `Synthetic Actor "${t.actor.name}" in Scene "${s.name}"`
          );
          if (update) updates.push(update);
        }
      }
